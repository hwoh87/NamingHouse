...
현재 상태(2026-08-16): 간격 토큰 준수율 **98.9%**, 4dp 격자 이탈 **0건**.
남은 원시값은 둘뿐이다 — `0.dp` 하나와 글줄 광학 정렬 보정용 `1.dp` 하나.

감사 시작 시점에는 준수율 0%, 격자 이탈 44%였다. 레퍼런스는 Gradle 캐시의
`material3-1.3.1` 샘플 소스를 같은 스크립트로 집계해 썼다(격자 이탈 3.6% ·
8dp 이하 마이크로 25.3% · 16dp 49.4%). 이 앱은 마이크로에 심하게 쏠려 있었고
16dp 자리를 20dp 가 대신하고 있었다.

정리는 네 단계로 나눠 커밋했다. **순서를 뒤집으면 깨진다** — 스냅을 먼저 하면
6·10·14 같은 정당한 값이 갈 곳이 없어 8·12·16 으로 뭉개진다.

| 단계 | 커밋 | 시각 변화 |
|---|---|---|
| ① 토큰 추가 | `1982d9c` | 없음 |
| ② 동일값 치환 120건 | `f445bd7` | 없음 |
| ③ 격자 스냅 62건 | `56e86c3` | **있음 — 되돌리려면 이것만 떼면 된다** |
| ④ 가드 | `8984590` | 없음 |

③ 의 규칙은 "가장 가까운 4dp, 같으면 올림"이고 매핑은 `tools/design-snap.py` 에
남아 있다. 올림을 고른 근거는 위 무게중심 실측이다. 상세·순위·보관함을 전후
같은 지점에서 스크린샷 대조해 무손상을 확인했다.

가드는 [HARD] 둘이다 — 토큰이 있는 값(2·4·8·12·16·20·24·28)을 원시 dp 로 되돌리면
실패하고, 격자 이탈이 하나라도 생기면 실패한다(`OFFGRID_BASELINE = 0`). 정당한
예외가 생기면 값을 `design-snap.py` 의 `KEEP` 에 넣고 이유를 남길 것.

터치 타겟은 가드하지 않는다. 이 Compose 세대에서는 맨 `Modifier.clickable` 도
48dp 최소 터치 영역으로 자동 확장되기 때문이다 — 기기 실측으로 5개 화면 87개
clickable 노드 전부 48dp 이상임을 확인했고, 시각 높이 13dp 인 표구 칩의 13dp
위를 탭해도 선택이 바뀌는 것까지 봤다. **코드만 읽고 터치 타겟 미달로 판단하지 말 것.**
