...
AVOID_SUBSTRINGS = (
    ...
    "게으를", "어지러울", "끊어질", "끊을", "발라낼", "시끄러울", "다툴", "싸울", "때릴", "찌를", "해칠", "독할",
...
NAME_HUN_KEYWORDS = (
    ...
    "붉을", "푸를", "검을", "희고", "찬란할",
    # 인품·덕
    # "바를"은 뺐다 — 正(곧다)과 剔(뼈 발라낼)·垸(회 바를)이 훈이 같아 부분일치로도
    # 낱말일치로도 갈라지지 않는다. 정당한 쪽(正·柾)만 NAME_CHAR_WHITELIST 로 통과시킨다.
    "착할", "슬기", "총명할", "지혜", "미쁠", "정성", "성실할", "곧을",
    "굳을", "굳셀", "씩씩할", "용감할", "부지런할", "힘쓸", "삼갈", "공손할", "온화할",
    # "바를"은 뺐다 — 正(곧다)과 剔(뼈 발라낼)·垸(회 바를)이 훈이 같아 갈라지지 않는다.
    # 정당한 쪽(正·柾)만 NAME_CHAR_WHITELIST 로 통과시킨다.
